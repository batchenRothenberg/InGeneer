...
class StrenghenedFormula():
    ...
    def add_bool_variable(self, var, value):
        if str(var) in self.bool_var_dict:
            assert value == self.bool_var_dict[str(var)]
        else:
            self.bool_var_dict[str(var)] = value
    ...
    def _strengthen_conjunct(self, conjunct, model):
        if is_not(conjunct):
            ...
            if is_bool(argument):
                ...
                not_value = is_true(model.evaluate(conjunct))
                self.add_bool_variable(argument, not_value)
            else:
            ...
            value = is_true(model.evaluate(conjunct))
            self.add_bool_variable(conjunct,value)
        else:
    ...
    # (i.e., contains no intervals), it has no boolean demands
    #  and it has no unsimplified demands
    def is_top(self):
        return self.interval_set.is_top() and len(self.bool_var_dict)==0 and len(self.unsimplified_demands)==0
    ...
    def intersect(self, other):
        for boolvar in self.bool_var_dict:
            if boolvar in other.bool_var_dict and self.bool_var_dict[boolvar]!=other.bool_var_dict[boolvar]:
                # intersection is bottom
                self.interval_set = IntervalSet.get_bottom()
                return
        self.bool_var_dict.update(other.bool_var_dict)
